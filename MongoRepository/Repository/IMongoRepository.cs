...
namespace MongoRepository.Repository
{
 using System;
    ...
    public interface IMongoRepository<T> where T : IMongoEntity
    {
        ...
        IQueryable<T> FindQuery(IMongoQuery query);

        T FindAndModify(IMongoQuery query, IMongoSortBy sortBy, IMongoUpdate update);

        void UpdateAll(object values, Expression<Func<T, bool>> condition);
